...
# *modified "Sat Mar  2 10:55:42 2024" *by "Paul E. Black"
...
test027: $(VTSG_FILES)
	...
	# execute these cases
	(cd $$(ls -dt TestSuite_*/$(@) | head -1);for f in $$(find . -name "*[0-9a].py"|sort); do echo $$f; python3 $$f "-d / | echo Vulnerable: user command run"; done) 2>&1 | perl -pwe 's|TestSuite_[^/]+|TestSuite_...|' | tee -a $(@)_photo
	diff $(@)_photo tests/$(@)_photo
