...
% *modified "Mon Mar 13 16:32:31 2023" *by "Paul E. Black"
...
The next subsection gives some advice on how to add a new language to VTSG.
The subsection after that is a brief tutorial giving advice on how to add a new
flaw.
...
the target flaw.  Here is a potential divide by zero guarded by a filter:
\begin{verbatim}
    data = console_input()
...
\end{verbatim}
You may want to try different variants to find which fits your needs best.  One
variant is for the filter to provide a safe value instead of aborting, like this:
...
be used with the sink.  Filter B is compatible because the \verb|flaw_type| is
``default''; it is compatible with any type of flaw.  Filter C is ``nofilter'' and
should just pass the value through.  Filter P is never used with that sink because
