...
% *modified "Fri Sep 23 17:20:44 2022" *by "Paul E. Black"
...
% zero-width space, so that -- does not become en dash
...
Under the flaw type directory are directories for safe or unsafe (vulnerable) test
cases, depending on
which are generated.
...
Many kinds of modules use the same attributes.  Instead of repeating documentation of
...
For example,
    \verb|<flaw flaw_type="CWE_89" safe="0"/>| means that the input is for the CWE 89
    and that it is never safe.  If an input
...
\begin{itemize}
    ...
    \item flaw: vulnerability categories where the sample can be used, that is, the
      flaws that the input is compatible with.  Also whether it is safe or unsafe
      for that vulnerability type.  If this input is generic and compatible with all
...
\begin{itemize}
    ...
    \item flaw: whether the Filter is compatible with a flaw type and if it is
    safe for this type.  For example, \\
    \verb|<flaw flaw_type="CWE_89" safe="1"/>| means that the filter is for
    CWE 89 and that it is always safe.  Giving \verb|safe=”0”| means that it is never
    safe.  It could be always safe or never safe for another flaw type.
    If a filter is compatible with
    all types of flaws, put \verb|ALL| in the \verb|flaw_type| attribute.
...
When adding complexity, VTSG can use several complexities in one
test case.
