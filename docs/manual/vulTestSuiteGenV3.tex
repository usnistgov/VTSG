...
% *modified "Wed Mar  1 16:21:36 2023" *by "Paul E. Black"
...
% VTSG V3 produces manifest files in the Static Analysis Results Interchange Format
...
VTSG's default is to generate both the unsafe (buggy or vulnerable)
...
associated with it.
They are combined and nested to create slightly more realistic source code.
...
\begin{itemize}
    ...
    \item output\_type: the type of output.  The variable generated with the
      ...
      An input is selected of it matches the \verb|input_type| of the Filter or
      Sink, if the Filter \verb|input_type| is ``nofilter''.
...
\begin{itemize}
    ...
    \item flaw: indicate whether this filter is always safe, always unsafe, or
      ...
      types of vulnerabilities, put ``ALL'' as the \verb|flaw_type|.
...
\begin{itemize}
    ...
    \item options: By default filters are wrapped in complexities.  Complexities may
      be disabled by adding \verb|options| with \verb|need_complexity="0"|. In other
      words, if \linebreak[4] \verb|need_complexity| is in the first
...
\begin{itemize}
    ...
    \item INPUT:  Input description (dirs) (optional)
...
Explain how to add stuff . . .

\subsection{How to Add a Language}

The easiest is to
\begin{enumerate}
\item decide the short name of the language, e.g., py for Python and php for
...
\end{enumerate}
Change language characteristics in all the files.  Execute the resulting source
code, to make sure syntax and other details are correct.  It's best to start by
converting just one or two sinks, inputs, and filters for quick turn around.
Comment out all the rest at the beginning.

\subsection{How to Add a Flaw}

A good first step is to take an existing generated test case and modify the
source code to present the target flaw.  Then decide what parts of the code are
in sinks, what parts are in filters (and wrapped in complexities), and what
parts are inputs.

Write the new flaw sink or sinks.  It's likely that nothing will be generated
the first time because VTSG cannot find any compatible filters and inputs.

Compatible inputs, filters, and sinks are ``connected'' various ways.  A filter
is used with a sink if
\begin{enumerate}
\item the filter \verb|output_type| is ``nofilter'' or the same as the sink
  \verb|input_type| \emph{and}
\item the filter has a \verb|flaw_type| that is ``ALL'' or is the same as the
  sink \verb|flaw_type|.
\end{enumerate}
...
\item the filter \verb|input_type| is not ``nofilter'' and the
...
This section explains the example test case used in this manual.  The case consists of
two files.
...
\hline
21 & while True code if condition break break \\
\hline
