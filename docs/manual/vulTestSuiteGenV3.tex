...
% *modified "Tue Feb  8 11:01:25 2022" *by "Paul E. Black"
...
VTS version 1 generates \CSharp\ programs. VTS version 2 generates PHP
...
The global project can be found at: \href{https://github.com/stivalet/Vulnerability-Test-Suite-generator}{https://github.com/stivalet/Vulnerability-Test-Suite-generator}\\
...
\noindent To copy the generic VTSG from GitHub to a local Linux machine, create a directory, called \textbf{src}, using the following command:

\begin{verbatim}
    mkdir src
\end{verbatim}

\noindent Go to the newly created \textbf{src} directory, by using the following command:
...
\noindent Then, from the \textbf{src} directory, type the following command to copy the GitHub source code to the \textbf{src} directory:

\begin{verbatim}
    git clone https://github.com/stivalet/Vulnerability-Test-Suite-generator.git
\end{verbatim}

\noindent If one types the \verb|ls -a| command, one will see that the \textbf{Vulnerability-Test-Suite-generator} directory was created.

\noindent Go to that directory, using the following command:

\begin{verbatim}
    cd Vulnerability-Test-Suite-generator
\end{verbatim}
...
There are two groups who will typically use VTS. The first group comprises people requiring test cases written in PHP, \CSharp, or Python to evaluate a static analyzer. These users must know how to invoke VTS with command line interface and retrieve the appropriate sample from the generated and categorized folders. 

The second targeted audience of the VTS is comprised of people wishing to generate test cases using a programming language other than the languages currently supported. The second group of users must use the template to construct the program using the XML tags, execute VTS with command line interface or another input method, and retrieve the samples from the generated and categorized file folders.

\subsection{Vulnerabilities Supported by VTS}
...
\begin{verbatim}
$ python3 test_suite_gen.py -l {php,cs,python} <options>
\end{verbatim}
...
cases, cs, for \CSharp\ cases, or python, \\
for Python cases. 
...
combinations are generated, VTS terminates.  The default, -1,
...
and VTS is invoked with \verb|-r 2|.  Each combination will have 
1 (no complexities) + 5 (each complexity, not nested) + 
25 (each complexities nested within every complexity) = 31 test cases.  
Hence VTS may generate \emph{far} more test cases than the number 
...
VTS V3 generates test cases from information in Template, Input, 
...
\subsection{VTS Directory Structure}
\label{sec: directory structure}

All these types of files are kept in a \verb|templates| subdirectory.
Under this is one subdirectory for each language.  VTS chooses the 
...
and \verb|exec_queries.xml|.  To add another language to VTS, 
...
VTS generates test cases with two broad steps.  First, VTS selects
...
CWE constraints the user gives on the command line.  Second, VTS
...
When VTS has selected a set of modules, it begins composing
...
Section~\ref{sec:case file name} describes how VTS names the file.

VTS generates many different test cases, both with and without 
flaws, with various control flow complexities.  After VTS finishes
...
VTS generates hundreds of test cases in minutes.

VTS is built to generate test cases with all consistent combinations
of modules for the flaw groups and CWEs specified in the invocation.
If VTS is invoked with flaw groups (\verb|-f|) or CWEs (\verb|-c|),
...
VTS generates test cases with all complexities up to the depth
indicated.
For example, the default depth, 1, leads VTS to generate all 
test cases with no flow complexities and all test cases with 
one complexity.  The option \verb|-r 2| leads VTS to generate 
...
These XML files are required when adding new functionality to VTS. 
There 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        does not appear in the test case code.  It tells VTS
...
\begin{itemize}
    ...
    underscore (``\_'').\\
    VTS V3 supports many database management systems, including ORACLE, \\
    MySQL,
...
When adding Complexity, VTS can use several complexity in one 
test case.
...
This section describes where VTS places test cases and what the
