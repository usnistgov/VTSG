...
% *modified "Tue Sep 12 16:30:24 2023" *by "Paul E. Black"
...
buffer overflows, SQL injections, noncompliance with an organization’s coding
...
We can measure how well static analyzers perform with programs having known
...
input data, varying sinks and where the data is passed along.
...
In addition it is tedious and error prone to construct these test cases
manually.
...
All example programs, or test cases, have a similar structure. In pseudocode,
the high-level structure of test cases that VTSG generates is
\begin{verbatim}
        declarations
        ...
        execute the query
\end{verbatim}
The input is the source of data in the program, e.g., command line, variable, file,
or form method.

VTSG refers to the code that checks the input as the \emph{filter}.  That is, it
...
VTSG refers to the code that uses the input, to construct a query in this example, as
the \emph{sink}.  Information flows from the input source to the sink. This is where
a sensitive operation, such as an array access, is executed with potentially unsafe
input and where a vulnerability is triggered.  You can see this high-level program
...
\begin{figure}[htbp]
  ...
  \caption{Overview of VTSG test case generation.  The File Template specifies how
    ...
    An example of a generated test case is in
  Figs.~\ref{fig:example main file} and \ref{fig:example aux file}.}
  \label{fig:VTS operation overview}
\end{figure}

In theory, a static analysis tool only needs to process the few lines of code that
embody a problematic piece of code to determine whether it is actually a weakness.
In practice however, a tool must analyze much of the program, noting its control and
data flows, to accurately track data and determine the conditions when the code in
question may be executed.  Tracing execution through structures like \verb|while|
...
executing the query constructing in the sink code.  VTSG refers to this as
\emph{ExecQuery}.

This manual is divided into several section.  The next section,
...
supporting packages, add new flaw classes, sinks, inputs, etc. to existing languages
and how to add a completely new language.
Finally, Sect.~\ref{sec:extending VTSG} explains the internal architecture of VTSG
and offers suggestions about how to enhance or extend VTSG's capabilities.

\subsection{Relation With Other SAMATE Work}

VTSG is further work by the SAMATE team to measure software assurance.  The test
cases that VTSG generates can help us determine how much assurance static analyzers,
and other software assurance tools, can bring.  This complements our work collecting
test cases in the Software Assurance Reference Dataset
(\href{https://samate.nist.gov/SARD}{SARD}) \cite{SARD2018}, available at
https://samate.nist.gov/SARD on-line.  VTSG also supports our Static Analysis Tool
Exposition
...
needed in any language, for any weakness class.
...
\begin{itemize}
  ...
  \item VTSG can produce a manageable fraction of all possible combinations, choosing
    1 of every N cases or enough cases to try every pair or triple of code.
\end{itemize}

VTSG is not nearly as powerful as we could wish for.  Here are some limitations and
possible future enhancements.
\begin{itemize}
  \item Have a method of variants.  For example, data types that are given once and a
  ...
  \item Enhance support for multi-file cases.  In particular, be able to handle
    Python classes.  Also create cases with more than two files, like Juliet cases.
  ...
  \item Allow the user to specify the file names of generated cases.  Currently the
    names are quite long.  It may be useful to structure the names differently or
...
can generate Python programs.

The PHP and \CSharp\ test suites are
available in NIST's Software Assurance Reference Dataset
(\href{https://samate.nist.gov/SARD}{SARD}), URL https://samate.nist.gov/SARD as
SARD test suites 103 and 105.
The SARD also has collections of test cases, such as, the C/C++ and Java Juliet
test suites as well as many other
...
\section{VTSG Advanced User Manual}
...
\verb|filters.xml|, \verb|sinks.xml|, and \verb|exec_queries.xml|.  The files
...
has \verb|file_rights.txt|, which is
...
VTSG code structure is broadly
\begin{verbatim}
...
definition declaration for the specific program language, the ``Input'' metadata
\verb|<code>| portion is added to the test case.  The ``Filtering'' metadata
\verb|<code>| portion, plus its \verb|<flaw type>| and safety indicator, are
added to the test case.  Next, the ``Sink'' metadata \verb|<code>| portion is
added to the test case.  Finally, the ``ExecQuery'' type is noted and the
\verb|<code>| portion of the ``ExecQuery'' is added to the test case.  The test
case is written to one or more files.  The location chosen for the files is
